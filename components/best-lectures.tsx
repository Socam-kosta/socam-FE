      ...
      try {
        ...
        if (!token) {
          ...
          const lecturesWithRating = await Promise.all(
            allLectures.slice(0, 20).map(async (lecture) => {
          ...
          const sortedLectures = lecturesWithRating
            .sort((a, b) => {
        ...
        // 평점순으로 정렬 (평점이 높은 순, 리뷰가 많은 순)
        // 리뷰가 없는 강의도 포함하되, 리뷰가 있는 강의를 우선으로 정렬
        const sortedLectures = lecturesWithRating
          .sort((a, b) => {
            // 리뷰가 있는 강의를 우선
            if (a.reviewCount === 0 && b.reviewCount > 0) return 1;
            ...
            return b.reviewCount - a.reviewCount; // 리뷰 많은 순
          })
          .slice(0, 4); // 상위 4개만

        setLectures(sortedLectures);
      } catch (error) {
        console.error("베스트 강의 로드 실패:", error);
        setLectures([]);
      } finally {
        setIsLoading(false);
      }
